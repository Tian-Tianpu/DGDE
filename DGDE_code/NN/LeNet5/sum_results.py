...
def summarize_DGDE_experiment_results(folder_path, excel_filename):
    all_data = []

    for root, dirs, files in os.walk(folder_path):
        for filename in files:
            if filename.endswith(".pkl") and ("DGDE_ADAM" in filename or "DGDE_SGD" in filename):
                file_path = os.path.join(root, filename)
                ...
                try:
                    ...
                    # 使用正则表达式从文件名中提取关键信息
                    match = re.match(r"results_([\d.]+)_([A-Za-z0-9.]+)_([\d.]+)_(DGDE_ADAM|DGDE_SGD)\.pkl", filename)
                    if match:
...
folder_path_dgde = "./Results_DGDE/"
excel_dgde_filepath = "./Results_DGDE/parameter_results.xlsx"
summarize_DGDE_experiment_results(folder_path_dgde, excel_dgde_filepath)
...
result_folders = ["./Results_GD", "./Results_AdaptiveGDE", "./Results_ESGD", "./Results_DGDE"]
