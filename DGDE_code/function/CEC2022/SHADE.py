        ...
        for idx in range(0, self.pop_size):
            ...
            while True:
                ...
                if np.any(x_r2.solution - x_r1.solution) and np.any(x_r2.solution - self.pop[idx].solution):
                    break
            x_new = self.pop[idx].solution + f * (x_best.solution - self.pop[idx].solution) + f * (
...
# miu_cr = 0.5
#
# # model = SHADE.OriginalSHADE(epoch=epoch, pop_size=pop_size, miu_f=miu_f, miu_cr=miu_cr)
# model = OriginalSHADE(epoch=epoch, pop_size=pop_size, miu_f=miu_f, miu_cr=miu_cr)
# model.solve(problem_dict)
#
#
# with open('./pickle/SHADE_model.pkl', 'wb') as f:
#     joblib.dump(model, f)
# with open('./pickle/SHADE_model.pkl', 'rb') as f:
#     loaded_model = joblib.load(f)
#
# # print(f"Solution: {loaded_model.g_best.solution}, Fitness: {loaded_model.g_best.target.fitness}")
...
# fitness_values = [agent.target.fitness for agent in loaded_model.pop]
# mean_fitness = np.mean(fitness_values)
# variance_fitness = np.var(fitness_values)
# print(fitness_values)
# print(f"Mean Fitness: {mean_fitness}")
# print(f"Variance of Fitness: {variance_fitness}")
