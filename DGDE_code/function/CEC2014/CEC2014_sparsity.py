...
def Rotated_High_Conditioned_Elliptic(x, M, o):
    ...
    if issparse(M):
        ...
        y = M @ v
    return High_Conditioned_Elliptic(y) + 100
...
def Rotated_High_Conditioned_Elliptic_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = M.T.dot(High_Conditioned_Elliptic_grad(y))
    else:
        y = M @ v
        grad = M.T @ High_Conditioned_Elliptic_grad(y)
...
def Rotated_Bent_Cigar_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = M.T.dot(Bent_Cigar_grad(y))
    else:
        y = M @ v
        grad = M.T @ Bent_Cigar_grad(y)
    return np.atleast_1d(grad).reshape(-1)
...
def Rotated_Discus_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = M.T.dot(Discus_grad(y))
    else:
        y = M @ v
        grad = M.T @ Discus_grad(y)
    return np.atleast_1d(grad).reshape(-1)
...
def Shifted_Rotated_Rosenbrock_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v) + 1
        grad = 0.02048 * M.T.dot(Rosenbrock_grad(y))
    else:
        y = M @ v + 1
        grad = 0.02048 * (M.T @ Rosenbrock_grad(y))
    return np.atleast_1d(grad).reshape(-1)
...
def Shifted_Rotated_Ackley_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = M.T.dot(Ackley_grad(y))
    else:
        y = M @ v
        grad = M.T @ Ackley_grad(y)
    return np.atleast_1d(grad).reshape(-1)
...
def Shifted_Rotated_Weierstrass_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 0.005 * M.T.dot(Weierstrass_grad(y))
    else:
        y = M @ v
        grad = 0.005 * (M.T @ Weierstrass_grad(y))
    return np.atleast_1d(grad).reshape(-1)
...
def Shifted_Rotated_Griewank_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 6 * M.T.dot(Griewank_grad(y))
    else:
        y = M @ v
        grad = 6 * (M.T @ Griewank_grad(y))
    return np.atleast_1d(grad).reshape(-1)
...
def Shifted_Rotated_Rastrigin_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 0.0512 * M.T.dot(Rastrigin_grad(y))
    else:
        y = M @ v
        grad = 0.0512 * (M.T @ Rastrigin_grad(y))
...
def Shifted_Rotated_Schwefel(x, M, o):
    ...
    return Modified_Schwefel(y) + 1100
...
def Shifted_Rotated_Schwefel_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 10 * M.T.dot(Modified_Schwefel_grad(y))
    else:
        y = M @ v
        grad = 10 * (M.T @ Modified_Schwefel_grad(y))
...
def Shifted_Rotated_Katsuura_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 0.05 * M.T.dot(Katsuura_grad(y))
    else:
        y = M @ v
        grad = 0.05 * (M.T @ Katsuura_grad(y))
...
def Shifted_Rotated_HappyCat_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 0.05 * M.T.dot(HappyCat_grad(y))
    else:
        y = M @ v
        grad = 0.05 * (M.T @ HappyCat_grad(y))
...
def Shifted_Rotated_HGBat_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 0.05 * M.T.dot(HGBat_grad(y))
    else:
        y = M @ v
        grad = 0.05 * (M.T @ HGBat_grad(y))
...
def Shifted_Rotated_Expanded_Griewank_plus_Rosenbrock(x, M, o):
    ...
    return Expended_Griewank_plus_Rosenbrock(y + 1) + 1500
...
def Shifted_Rotated_Expanded_Griewank_plus_Rosenbrock_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = 0.05 * M.T.dot(
            Expended_Griewank_plus_Rosenbrock_grad(y + 1)
        )
    else:
        y = M @ v
        grad = 0.05 * (
            M.T @ Expended_Griewank_plus_Rosenbrock_grad(y + 1)
        )
...
def Shifted_Rotated_Expanded_Scaffer_F6(x, M, o):
    ...
    return Expanded_Scaffer_F6(y + 1) + 1600
...
def Shifted_Rotated_Expanded_Scaffer_F6_grad(x, M, o):
    ...
    if issparse(M):
        y = M.dot(v)
        grad = M.T.dot(
            Expanded_Scaffer_F6_grad(y + 1)
        )
    else:
        y = M @ v
        grad = M.T @ Expanded_Scaffer_F6_grad(y + 1)

    return np.atleast_1d(grad).reshape(-1)

# 假设这些函数已经定义好，如果没有，请确保已导入
# from cec2014_functions import Modified_Schwefel, Rastrigin, High_Conditioned_Elliptic, ...
...
class HybridBase:
    ...
    def __init__(self, M, o, seed=2):
        ...
        # 统一使用 shape 属性，不依赖 ndim
        m_shape = self.M.shape
        m_ndim = len(m_shape)

        if m_ndim == 3:
            self.cf_num = m_shape[0]
            self.nx = m_shape[1]
            self.blocks = [self.M[i] for i in range(self.cf_num)]
        elif m_ndim == 2:
            rows, cols = m_shape
            self.nx = cols
            if rows == cols:
                ...
                # 稀疏切片优化
                self.blocks = [self.M[i * self.nx: (i + 1) * self.nx, :] for i in range(self.cf_num)]
            else:
                raise ValueError(f"M shape {m_shape} 不符合逻辑")
        else:
            raise ValueError(f"M 必须是 2D 或 3D。当前 shape: {m_shape}")

        # 解析偏移向量 o
        if self.o.size == self.cf_num * self.nx:
            self.o_blocks = self.o.reshape(self.cf_num, self.nx)
        else:
            # 鲁棒性处理：如果 o 维度不够，补零
            self.o_blocks = np.zeros((self.cf_num, self.nx))
            if self.o.size >= self.nx:
                for i in range(self.cf_num):
                    self.o_blocks[i, :min(self.o.size, self.nx)] = self.o.flatten()[:self.nx]

        self.dim = self.nx
        self.perm = self.rng.permutation(self.dim)

    def _safe_matvec(self, R, v):
        """深度优化：矩阵-向量乘法短路逻辑"""
        m = len(v)
        # 如果 R 是稀疏单位阵或其切片，直接返回 v
        if issparse(R):
            # 检查是否为单位阵的简易判断（针对 10000D 优化）
            if R.nnz == m:
                return v
            return R.dot(v)
        else:
            # 稠密矩阵判断
            if m > 100 and np.allclose(R, np.eye(m)):
                return v
            return R @ v
    ...
    def _get_slices(self):
        # 这里的比例可以根据 CEC 标准调整
        d = self.dim
        return (slice(0, int(0.3 * d)),
                slice(int(0.3 * d), int(0.6 * d)),
                slice(int(0.6 * d), d))


class Hybrid1(HybridBase):
    def f(self, x):
        x_perm = np.asarray(x).flatten()[self.perm]
        slices = self._get_slices()
        res = 0.0

        # 子函数列表及其对应的函数名
        funcs = [Modified_Schwefel, Rastrigin, High_Conditioned_Elliptic]

        for i, slc in enumerate(slices):
            m = slc.stop - slc.start
            if m <= 0: continue

            # 获取当前子块的矩阵和偏移
            # 使用 min 避免 cf_num 不足导致的越界
            R = self.blocks[min(i, self.cf_num - 1)][:m, :m]
            o = self.o_blocks[min(i, self.cf_num - 1), :m]

            y = self._safe_matvec(R, x_perm[slc] - o)
            res += funcs[i](y)
    ...
    def grad(self, x):
        ...
        # 梯度函数列表
        grads = [Modified_Schwefel_grad, Rastrigin_grad, High_Conditioned_Elliptic_grad]
        ...
        for i, slc in enumerate(slices):
            ...
            if issparse(R):
                if R.nnz == m:  # 单位阵优化
                    ...
                    grad_perm[slc] = R.T.dot(g)
            else:
                grad_perm[slc] = R.T @ g

        out_grad = np.zeros_like(x_perm)
        out_grad[self.perm] = grad_perm
        return out_grad


class Hybrid2(HybridBase):
    def f(self, x):
        x_perm = np.asarray(x).flatten()[self.perm]
        slices = self._get_slices()
        res = 0.0
        funcs = [Bent_Cigar, HGBat, Rastrigin]
    ...
    def grad(self, x):
        ...
        slices = self._get_slices()
        grad_perm = np.zeros_like(x_perm)
        grads = [Bent_Cigar_grad, HGBat_grad, Rastrigin_grad]
        ...
        for i, slc in enumerate(slices):
            ...
            if issparse(R) and R.nnz == m:
                grad_perm[slc] = g
            else:
                grad_perm[slc] = R.T.dot(g) if issparse(R) else R.T @ g
...
#
#         if M.ndim == 3:
#             self.blocks = [M[i] for i in range(M.shape[0])]
#             self.cf_num = M.shape[0]
#             self.nx = M.shape[1]
#         elif M.ndim == 2:
#             rows, cols = M.shape
#             if rows == cols:
...
#                 self.cf_num = rows // cols
#                 self.nx = cols
#                 self.blocks = [M[i*self.nx:(i+1)*self.nx, :] for i in range(self.cf_num)]
#             else:
...
#             self.o_blocks = o.reshape(self.cf_num, self.nx)
#         elif o.ndim == 2 and o.shape == (self.cf_num, self.nx):
#             self.o_blocks = o
#         elif o.ndim == 1 and o.size == self.nx:
...
#         if m > 0:
#             R = self.blocks[0][:m, :m]
#             v = x_perm[idx1] - self.o_blocks[0, :m]
#             y = R.dot(v) if issparse(R) else R @ v
#             res += Modified_Schwefel(y)
#
#         # 子函数 2
#         m = idx2.stop - idx2.start
#         if m > 0:
#             R = self.blocks[min(1, len(self.blocks)-1)][:m, :m]
#             v = x_perm[idx2] - self.o_blocks[min(1, self.o_blocks.shape[0]-1), :m]
#             y = R.dot(v) if issparse(R) else R @ v
#             res += Rastrigin(y)
#
#         # 子函数 3
...
#         if m > 0:
#             R = self.blocks[min(2, len(self.blocks)-1)][:m, :m]
#             v = x_perm[idx3] - self.o_blocks[min(2, self.o_blocks.shape[0]-1), :m]
#             y = R.dot(v) if issparse(R) else R @ v
...
#             v = x_perm[idx1] - self.o_blocks[0, :m]
#             y = R.dot(v) if issparse(R) else R @ v
#             g = Modified_Schwefel_grad(y)
#             grad_perm[idx1] = R.T.dot(g) if issparse(R) else R.T @ g
#
#         # grad 子2
#         m = idx2.stop - idx2.start
#         if m > 0:
...
#             y = R.dot(v) if issparse(R) else R @ v
#             g = Rastrigin_grad(y)
#             grad_perm[idx2] = R.T.dot(g) if issparse(R) else R.T @ g
#
#         # grad 子3
#         m = idx3.stop - idx3.start
#         if m > 0:
...
#             v = x_perm[idx3] - self.o_blocks[min(2, self.o_blocks.shape[0]-1), :m]
#             y = R.dot(v) if issparse(R) else R @ v
#             g = High_Conditioned_Elliptic_grad(y)
#             grad_perm[idx3] = R.T.dot(g) if issparse(R) else R.T @ g
#
#         grad = np.zeros_like(x)
...
def Hybrid_1(x, M, o, seed=2):
    return Hybrid1(M, o, seed=seed).f(x)

def Hybrid_1_grad(x, M, o, seed=2):
    return Hybrid1(M, o, seed=seed).grad(x)
#
# # ---------- 工具：从 M/o 构建 blocks 与 o_blocks ----------
# from scipy.sparse import issparse

def _parse_blocks_and_shifts(M, o):
    # --- 修改点 1: 仅对非稀疏对象使用 np.asarray ---
    if not issparse(M):
        M = np.asarray(M)
    o = np.asarray(o)

    # --- 修改点 2: 统一获取维度信息 ---
    # 无论是 numpy 还是 scipy.sparse，都支持 .shape
    shape = M.shape
    ndim = len(shape)

    # parse M into list of blocks and determine cf_num and nx
    if ndim == 3:
        # 稀疏矩阵通常是 2D，但为了兼容性保留 3D 处理
        blocks = [M[i].copy() for i in range(shape[0])]
        cf_num = shape[0]
        nx = shape[1]
    elif ndim == 2:
        rows, cols = shape
        if rows == cols:
            ...
            cf_num = rows // cols
            nx = cols
            # --- 修改点 3: scipy.sparse 支持这种切片，且不会转为稠密 ---
            blocks = [M[i*nx:(i+1)*nx, :].copy() for i in range(cf_num)]
        else:
            raise ValueError(f"Unexpected M shape {shape}")
    else:
        # 如果是稀疏矩阵但 ndim 被识别为 0，提供更详细的错误提示
        raise ValueError(f"M must be 2D or 3D array. Current type: {type(M)}, Shape: {shape}")
...
class Hybrid3:
    ...
    def f(self, x):
        ...
        if m > 0:
            R = self.blocks[min(3, len(self.blocks)-1)][:m, :m]
            v = x_perm[idx4] - self.o_blocks[min(3, self.o_blocks.shape[0]-1), :m]
            y = R.dot(v) if issparse(R) else R @ v
            res += Expanded_Scaffer_F6(y)
    ...
    def grad(self, x):
        ...
        if m > 0:
            ...
            v = x_perm[idx4] - self.o_blocks[min(3, self.o_blocks.shape[0]-1), :m]
            y = R.dot(v) if issparse(R) else R @ v
            g = Expanded_Scaffer_F6_grad(y)
            grad_perm[idx4] = R.T.dot(g) if issparse(R) else R.T @ g
...
class Hybrid4:
    ...
    def f(self, x):
        ...
        if m > 0:
            ...
            v = x_perm[idx3] - self.o_blocks[min(2, self.o_blocks.shape[0]-1), :m]
            y = R.dot(v) if issparse(R) else R @ v
            res += Expended_Griewank_plus_Rosenbrock(y)
    ...
    def grad(self, x):
        ...
        if m > 0:
            ...
            v = x_perm[idx3] - self.o_blocks[min(2, self.o_blocks.shape[0]-1), :m]
            y = R.dot(v) if issparse(R) else R @ v
            g = Expended_Griewank_plus_Rosenbrock_grad(y)
            grad_perm[idx3] = R.T.dot(g) if issparse(R) else R.T @ g
...
class Hybrid5:
    ...
    def f(self, x):
        ...
        funcs = [Expanded_Scaffer_F6, HGBat, Rosenbrock,
        ...
        for i, idx in enumerate(idxs):
            ...
            if m <= 0:
                continue
            R = self.blocks[min(i, len(self.blocks)-1)][:m, :m]
            v = x_perm[idx] - self.o_blocks[min(i, self.o_blocks.shape[0]-1), :m]
            y = R.dot(v) if issparse(R) else R @ v
            res += funcs[i](y)
    ...
    def grad(self, x):
        ...
        x_perm = x[self.perm]
        idxs = self._slice_indices()
        grads = [Expanded_Scaffer_F6_grad, HGBat_grad, Rosenbrock_grad,
                 Modified_Schwefel_grad, High_Conditioned_Elliptic_grad]
        grad_perm = np.zeros_like(x_perm)
        ...
        for i, idx in enumerate(idxs):
            ...
            y = R.dot(v) if issparse(R) else R @ v
            g = grads[i](y)
            grad_perm[idx] = R.T.dot(g) if issparse(R) else R.T @ g
...
class Hybrid6:
    ...
    def f(self, x):
        ...
        x_perm = x[self.perm]
        idxs = self._slice_indices()
        funcs = [Katsuura, HappyCat,
                 Expended_Griewank_plus_Rosenbrock,
                 Modified_Schwefel, Ackley]
        res = 0.0
    ...
    def grad(self, x):
        ...
        x_perm = x[self.perm]
        idxs = self._slice_indices()
        grads = [Katsuura_grad, HappyCat_grad,
                 Expended_Griewank_plus_Rosenbrock_grad,
                 Modified_Schwefel_grad, Ackley_grad]
        grad_perm = np.zeros_like(x_perm)
...
#     # fix M shape
#     if M.shape[0] < 2*dim:
#         raise ValueError("Rotation matrix M size incorrect")
#     M = M[:2*dim, :]
#
#     sig = np.array([20.0, 20.0, 20.0])**2
#
...
#
#     diff0 = x - o[0,:]
#     f1 = Modified_Schwefel(diff0) - 1000
#     g1 = Modified_Schwefel_grad(diff0)
#
#     fvals[0] = f1
#     grads[0] = g1
#
...
#     dim = x.size
#
#     # 2. 严格对齐 Composition_3 的 sigmas [10, 30, 50]
#     sig = np.square(np.array([10.0, 30.0, 50.0], dtype=float))
#
#     # 3. 计算权重 (确保 o 是 2D 数组)
#     # 如果 o 只有 1000D 且没 reshape，这里会报错，强制转为 (3, 1000)
#     o_reshaped = o.reshape(3, dim)
#     w, grad_w = composition_omega_and_grad(x, o_reshaped, sig)
#
#     fvals = np.zeros(3, dtype=float)
#     grads = [None] * 3
#
#     # ---- 子函数 0: Modified Schwefel (对应 0.25 倍率) ----
#     M0 = M[0:dim, :]
#     diff0 = (x - o_reshaped[0]).flatten()  # 强制为向量
#     y0 = M0 @ diff0
#
#     fvals[0] = 0.25 * Modified_Schwefel(y0) - 275.0
#     grads[0] = M0.T @ (0.25 * Modified_Schwefel_grad(y0).flatten())
#
#     # ---- 子函数 1: Rastrigin ----
#     M1 = M[dim:dim * 2, :]
#     diff1 = (x - o_reshaped[1]).flatten()
#     y1 = M1 @ diff1
#
#     fvals[1] = Rastrigin(y1) - 800.0
#     grads[1] = M1.T @ Rastrigin_grad(y1).flatten()
#
#     # ---- 子函数 2: High Conditioned Elliptic (对应 1e-7 倍率) ----
#     M2 = M[dim * 2:dim * 3, :]
#     diff2 = (x - o_reshaped[2]).flatten()
#     y2 = M2 @ diff2
#
#     fvals[2] = 1e-7 * High_Conditioned_Elliptic(y2) - 1e-5 + 200.0
#     grads[2] = M2.T @ (1e-7 * High_Conditioned_Elliptic_grad(y2).flatten())
#
#     # ---- 4. 最终合成 ----
#     grad_total = np.zeros(dim, dtype=float)
#     for i in range(3):
...
def Composition_3_grad(x, M, o):
    """
    针对 CEC2014 Composition Function 3 的准确梯度实现。

    逻辑：
    - f1: 0.25 * Shifted_Rotated_Schwefel(x) - 275
    - f2: Shifted_Rotated_Rastrigin(x) - 800
    - f3: 1e-7 * Rotated_High_Conditioned_Elliptic(x) + 199.99999
    """
    import numpy as np
    ...
    # 2. 获取权重 w 和权重梯度 grad_w
    # Composition_3 使用的 sigma 为 [10, 30, 50]
    sig = np.square(np.array([10.0, 30.0, 50.0], dtype=float))

    # 确保 o 为 (3, dim) 形状
    if o.ndim == 1:
        o_2d = o.reshape(3, dim)
    else:
        o_2d = o
    ...
    # ---- 子函数 0: Modified Schwefel (对应 0.25 倍率) ----
    # 注意：Shifted_Rotated_Schwefel 内部逻辑是 y = M @ (10 * (x - o))
    # 但根据 Composition_3 的定义，通常是直接旋转偏移量 y = M @ (x - o)
    M0 = M[0:dim, :]
    diff0 = x - o_2d[0]
    y0 = M0 @ diff0

    fvals[0] = 0.25 * Modified_Schwefel(y0) - 275.0
    # 链式法则：∇f = M.T @ ∇Modified_Schwefel * 倍率
    grads[0] = M0.T @ (0.25 * Modified_Schwefel_grad(y0))
    ...
    fvals[1] = Rastrigin(y1) - 800.0
    grads[1] = M1.T @ Rastrigin_grad(y1)
    ...
    # 根据定义：1e-7 * f - 1e-5 + 200
    fvals[2] = 1e-7 * High_Conditioned_Elliptic(y2) - 1e-5 + 200.0
    grads[2] = M2.T @ (1e-7 * High_Conditioned_Elliptic_grad(y2))

    # ---- 3. 最终梯度合成 ----
    # 依据乘法法则：∇(Σ w_i * f_i) = Σ (∇w_i * f_i + w_i * ∇f_i)
    grad_total = np.zeros(dim, dtype=float)
    for i in range(3):
        grad_total += grad_w[i] * fvals[i] + w[i] * grads[i]
...
def Composition_4_grad(x, M, o):
    ...
    # sigs: [10, 10, 10, 10, 10] squared
    sig = np.array([10.0] * 5) ** 2

    # 计算权重 w (5,) 和权重梯度 grad_w (5, dim)
    # 确保你的 composition_omega_and_grad 内部也支持稀疏处理或只处理向量
    w, grad_w = composition_omega_and_grad(x, o, sig)

    fvals = np.zeros(5, dtype=float)
    grads = [None] * 5

    # ---- 核心修改：移除 np.ascontiguousarray，直接切片 ----
    # 对于 scipy.sparse.csr_matrix，切片操作 M[start:end, :] 是高效的且保持稀疏

    # 0: 0.25 * Shifted_Rotated_Schwefel - 275
    M0 = M[0:dim, :]
    y0 = M0 @ (x - o[0])
    fvals[0] = 0.25 * Modified_Schwefel(y0) - 275.0
    grads[0] = M0.T @ (0.25 * Modified_Schwefel_grad(y0))

    # 1: Shifted_Rotated_HappyCat - 1200
    M1 = M[dim:dim * 2, :]
    y1 = M1 @ (x - o[1])
    fvals[1] = HappyCat(y1) - 1200.0
    grads[1] = M1.T @ HappyCat_grad(y1)

    # 2: 1e-7 * Rotated_HighConditionedElliptic + 199.99999
    M2 = M[dim * 2:dim * 3, :]
    y2 = M2 @ (x - o[2])
    fvals[2] = 1e-7 * High_Conditioned_Elliptic(y2) + 199.99999
    grads[2] = M2.T @ (1e-7 * High_Conditioned_Elliptic_grad(y2))

    # 3: 2.5 * Shifted_Rotated_Weierstrass - 1200
    M3 = M[dim * 3:dim * 4, :]
    y3 = M3 @ (x - o[3])
    fvals[3] = 2.5 * Weierstrass(y3) - 1200.0
    grads[3] = M3.T @ (2.5 * Weierstrass_grad(y3))

    # 4: 10 * Shifted_Rotated_Griewank - 6600
    M4 = M[dim * 4:dim * 5, :]
    y4 = M4 @ (x - o[4])
    fvals[4] = 10.0 * Griewank(y4) - 6600.0
    grads[4] = M4.T @ (10.0 * Griewank_grad(y4))

    # 组合最终梯度
    grad_total = np.zeros(dim, dtype=float)
    for i in range(5):
        # 此时 w[i] 是标量，fvals[i] 是标量，grads[i] 是稠密向量，grad_w[i] 是稠密向量
        grad_total += grad_w[i] * fvals[i] + w[i] * grads[i]
...
def Composition_5_grad(x, M, o):
    ...
    grads[0] = M0.T @ (10.0 * HGBat_grad(y0))


    # ---------- Component 1 : Rastrigin ----------
    M1 = M[dim:dim*2, :]
    y1 = M1 @ (x - o[1])

    fvals[1] = 10.0 * Rastrigin(y1) - 8900.0
    grads[1] = M1.T @ (10.0 * Rastrigin_grad(y1))


    # ---------- Component 2 : Modified Schwefel ----------
    M2 = M[dim*2:dim*3, :]
    y2 = M2 @ (x - o[2])

    fvals[2] = 2.5 * Modified_Schwefel(y2) - 1000.0
    grads[2] = M2.T @ (2.5 * Modified_Schwefel_grad(y2))


    # ---------- Component 3 : Weierstrass ----------
    M3 = M[dim*3:dim*4, :]
    y3 = M3 @ (x - o[3])

    fvals[3] = 2.5 * Weierstrass(y3) - 1700.0
    grads[3] = M3.T @ (2.5 * Weierstrass_grad(y3))


    # ---------- Component 4 : Elliptic ----------
    M4 = M[dim*4:dim*5, :]
    y4 = M4 @ (x - o[4])

    fvals[4] = 1e-6 * High_Conditioned_Elliptic(y4) + 399.9999
    grads[4] = M4.T @ (1e-6 * High_Conditioned_Elliptic_grad(y4))
...
#     grads = [None] * 5
#
#     # 修改点：直接切片，不使用 ascontiguousarray
#     # Component 0: HGBat
#     M0 = M[0:dim, :]
#     y0 = M0 @ (x - o[0])
#     fvals[0] = 10.0 * HGBat(y0) - 14000.0
...
#     grads[1] = M1.T @ (10.0 * Rastrigin_grad(y1))
#
#     # ... 后续组件 2, 3, 4 同理修改为 M[i*dim:(i+1)*dim, :] ...
#     # Component 4: Elliptic
#     M4 = M[dim*4:dim*5, :]
#     y4 = M4 @ (x - o[4])
#     fvals[4] = 1e-6 * High_Conditioned_Elliptic(y4) + 399.9999
...
def Composition_6_grad(x, M, o):
    """
    修正后的 Composition_6 梯度函数。
    支持 1000D/10000D 稀疏旋转矩阵 M。
    """
    x = np.asarray(x, dtype=float).flatten()
    dim = x.size
    ...
    # --- 核心修正：创建稀疏单位矩阵替代 np.eye(dim) ---
    # 在 10000D 下，np.eye 占用 800MB，而 sp.eye 仅占用几 KB
    identity_sparse = sp.eye(dim, format='csr')
    zero_o = np.zeros(dim)

    # 组件 0: Shifted_Rotated_Expanded_Griewank_plus_Rosenbrock
    M0 = M[0:dim, :]  # 保持稀疏切片
    diff0 = x - o[0, :]
    y0 = M0 @ diff0
    # 注意：这里传入 identity_sparse 和 zero_o 是因为旋转和偏移已经在 y0 中处理过了
    fvals[0] = 2.5 * Shifted_Rotated_Expanded_Griewank_plus_Rosenbrock(y0, identity_sparse, zero_o) - 3750.0
    gy0 = 2.5 * Shifted_Rotated_Expanded_Griewank_plus_Rosenbrock_grad(y0, identity_sparse, zero_o)
    grads[0] = M0.T @ gy0

    # 组件 1: Shifted_Rotated_HappyCat
    M1 = M[dim:dim*2, :]
    diff1 = x - o[1, :]
    y1 = M1 @ diff1
    fvals[1] = 10.0 * Shifted_Rotated_HappyCat(y1, identity_sparse, zero_o) - 12900.0
    gy1 = 10.0 * Shifted_Rotated_HappyCat_grad(y1, identity_sparse, zero_o)
    grads[1] = M1.T @ gy1

    # 组件 2: Shifted_Rotated_Schwefel
    M2 = M[dim*2:dim*3, :]
    diff2 = x - o[2, :]
    y2 = M2 @ diff2
    fvals[2] = 2.5 * Shifted_Rotated_Schwefel(y2, identity_sparse, zero_o) - 2550.0
    gy2 = 2.5 * Shifted_Rotated_Schwefel_grad(y2, identity_sparse, zero_o)
    grads[2] = M2.T @ gy2

    # 组件 3: Shifted_Rotated_Expanded_Scaffer_F6
    M3 = M[dim*3:dim*4, :]
    diff3 = x - o[3, :]
    y3 = M3 @ diff3
    fvals[3] = 5e-4 * Shifted_Rotated_Expanded_Scaffer_F6(y3, identity_sparse, zero_o) + 299.2
    gy3 = 5e-4 * Shifted_Rotated_Expanded_Scaffer_F6_grad(y3, identity_sparse, zero_o)
    grads[3] = M3.T @ gy3

    # 组件 4: Rotated_High_Conditioned_Elliptic
    M4 = M[dim*4:dim*5, :]
    diff4 = x - o[4, :]
    y4 = M4 @ diff4
    fvals[4] = 1e-6 * Rotated_High_Conditioned_Elliptic(y4, identity_sparse, zero_o) + 399.9999
    gy4 = 1e-6 * Rotated_High_Conditioned_Elliptic_grad(y4, identity_sparse, zero_o)
    grads[4] = M4.T @ gy4
...
def Composition_7_grad(x, M, o):
    """
    针对 1000D/10000D 优化的 Composition_7 梯度函数。
    支持稀疏矩阵 M。
    """
    x = np.asarray(x, dtype=float).flatten()
    dim = x.size
    ...
    # --- 修正点：直接进行稀疏切片，不使用 np.ascontiguousarray ---

    # 组件 0: Hybrid_1
    M0 = M[0:dim, :]
    fvals[0] = Hybrid_1(x, M0, o[0, :]) - 1700.0
    grads[0] = Hybrid_1_grad(x, M0, o[0, :])  # 内部应处理 M0 @ diff
    ...
    grad_total = np.zeros(dim, dtype=float)
    for i in range(3):
        # grad_w[i] 和 grads[i] 此时均为稠密向量 (dim,)
        grad_total += grad_w[i] * fvals[i] + w[i] * grads[i]
...
def Composition_8_grad(x, M, o):
    """
    针对 1000D/10000D 优化的 Composition_8 梯度函数。
    支持稀疏矩阵 M。
    """
    x = np.asarray(x, dtype=float).flatten()
    dim = x.size
